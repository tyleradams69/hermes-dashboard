...
- Delivery focus queue on `/businesses`: `src/lib/clientWorkspace.ts` derives readiness score/tier, launch blockers, and next-step guidance so stalled client work surfaces before launched work.
- Copy-ready delivery controls on `/businesses`: each delivery workspace can copy an internal handoff summary or an action plan generated by `formatClientDeliveryActionPlan()`.
